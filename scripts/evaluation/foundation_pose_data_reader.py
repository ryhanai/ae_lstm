...
# sys.path.append("/home/ryo/Program/moonshot/ae_lstm/scripts")
# from core.object_loader import ObjectInfo
# object_info = ObjectInfo(dataset="ycb_conveni_v1", split="test")

cam_K_file = "/home/ryo/Program/moonshot/ae_lstm/specification/config/foundation_pose_cam_K.txt"


def get_obj_file(name):
    return f"/home/ryo/Dataset/ycb_video/models/{name}/textured_simple.obj"
...
class YcbineoatReader:
    def __init__(self, video_dir, target, downscale=1, shorter_side=None, zfar=np.inf):
        self.video_dir = video_dir
        self.target = target
        self.downscale = downscale
        self.zfar = zfar
        # self.color_files = sorted(glob.glob(f"{self.video_dir}/rgb/*.png"))
        self.color_files = sorted(glob.glob(f"{video_dir}/*-color.jpg"))
    ...
    def get_color(self, i):
        color = cv2.imread(self.color_files[i], cv2.IMREAD_UNCHANGED)
        color = cv2.cvtColor(color, cv2.COLOR_BGR2RGB)
        return color

    def get_mask(self, i):
        # mask = cv2.imread(self.color_files[i].replace("rgb", "masks"), -1)
        mask = cv2.imread(self.color_files[i].replace("-color.jpg", f"-label-{self.target}.png"), -1)
        # mask = cv2.imread("/home/ryo/Program/moonshot/ae_lstm/scripts/evaluation/mask.png", -1)
        # if len(mask.shape) == 3:
        #     for c in range(3):
        #         if mask[..., c].sum() > 0:
        #             mask = mask[..., c]
    ...
    def get_depth(self, i):
        depth = cv2.imread(self.color_files[i].replace("-color.jpg", "-depth.png"), -1) / 1e3
        depth[(depth < 0.1) | (depth >= self.zfar)] = 0
        # depth[(depth < 0.5) | (depth >= 1.0)] = 0
        return depth
    ...
    def get_gt_mesh(self):
        ...
        # mesh = trimesh.load(object_info.obj_file(self.target)[0])
        mesh = trimesh.load(get_obj_file(self.target)[0])
        return mesh
