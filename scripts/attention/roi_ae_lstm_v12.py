...
def model_roi_mlp(latent_vec_dim, name='roi_mlp'):
    latent_vec_input = tf.keras.Input(shape=(latent_vec_dim))

    roi_param = tf.keras.layers.Dense(3, activation='sigmoid')(latent_vec_input) # (x, y, s)
    ...
    m = keras.Model(latent_vec_input, roi, name=name)
    m.summary()
    return m
...
def model_roi_ae_lstm(input_image_shape, time_window_size, image_vec_dim, dof):
    ...
    encoder = model_encoder(input_image_shape, time_window_size, image_vec_dim)
    lstm = model_lstm_no_split(time_window_size, image_vec_dim, dof)
    
    encoded_img = encoder(image_input)
    predicted = lstm([encoded_img, joint_input])
    roi = model_roi_mlp(image_vec_dim+dof)(predicted)

    # crop&resize ROI
    roi_img = tf.keras.layers.TimeDistributed(tf.keras.layers.Lambda(crop_and_resize, output_shape=input_image_shape)) ([image_input, roi])

    encoded_roi = encoder(roi_img)
    predicted2 = lstm([encoded_roi, joint_input])

    # separete latent vector
    predicted_ivec = tf.keras.layers.Lambda(lambda x:x[:,:image_vec_dim], output_shape=(image_vec_dim,))(predicted2)
    predicted_jvec = tf.keras.layers.Lambda(lambda x:x[:,image_vec_dim:], output_shape=(dof,))(predicted2)
