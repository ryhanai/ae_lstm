...
def roi_fun():
    ...
    v = np.array([0.2, 0.2])
    es = np.random.random()
    ep = np.random.random(2)
    es = 0.7 + 0.3*es
    ep = 2. * (ep - 0.5)
    left_top = center + v*ep - a*es
    right_bottom = center + v*ep + a*es
    roi = np.concatenate((left_top, right_bottom))
    return roi
...
class GeneratorTest:
    def __init__(self):
        ...
        self.train_gen = train_generator.flow(train_ds, const_roi_fun, batch_size=self.batch_size, time_window_size=self.time_window)
    def batch(self, n=10):
        start = time.time()
        for i in range(n):
...
class ROI_AE_LSTM_Trainer:
    ...
    def load_val_data(self):
        ...
        self.val_gen = val_generator.flow(val_ds, const_roi_fun, batch_size=self.batch_size, time_window_size=self.time_window)
