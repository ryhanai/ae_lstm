...
def expand_time_dim(args):
    args_ex = tf.expand_dims(args, 0)
    return tf.transpose(tf.tile(args_ex, tf.constant([time_window_size, 1, 1], tf.int32)), [1,0,2])
...
def model_lstm_no_split(time_window_size, image_vec_dim, dof, lstm_units=50, use_stacked_lstm=False, name='lstm'):
    imgvec_input = tf.keras.Input(shape=(time_window_size, image_vec_dim))
    joint_input = tf.keras.Input(shape=(time_window_size, dof))
    roi_param_input = tf.keras.Input(shape=(time_window_size, 3))
    state_dim = image_vec_dim + dof

    joint_input_noise = tf.keras.layers.GaussianNoise(0.03)(joint_input)
    
    x = tf.keras.layers.concatenate([imgvec_input, joint_input_noise, roi_param_input])
    ...
    lstm = keras.Model([imgvec_input, joint_input, roi_param_input], x, name=name)
    lstm.summary()
    return lstm
...
def model_prediction(input_image_shape, time_window_size, image_vec_dim, dof):
    ...
    # convert an ROI param to a time sequence of rectangular regions
    ...
    # convert an ROI param to a time sequence of the ROI params
    roi_param_time_seq = tf.keras.layers.Lambda(expand_time_dim)(roi_input)
    ...
    # LSTM
    predicted_state = model_lstm_no_split(time_window_size, image_vec_dim, dof)([roi_encoding, joint_input, roi_param_time_seq])
...
def train():
    ...
    # cp='ae_cp.reaching.predictor.20220309145305'
...
def train_roi_estimator(predictor_cp='ae_cp.reaching.predictor.20220309145305'):
    train_ds = Dataset(dataset)
    train_ds.load(groups=train_groups, image_size=input_image_size)
    train_ds.preprocess(time_window_size)
    val_ds = Dataset(dataset)
...
def prepare_for_predictor_test(cp='ae_cp.reaching.predictor.20220309145305'):
    val_ds = Dataset(dataset)
    val_ds.load(groups=val_groups, image_size=input_image_size)
    val_ds.preprocess(time_window_size)
...
def prepare_for_test(cp='ae_cp.reaching.roi_estimator.20220309185120'):
    val_ds = Dataset(dataset)
    val_ds.load(groups=val_groups, image_size=input_image_size)
    val_ds.preprocess(time_window_size)
