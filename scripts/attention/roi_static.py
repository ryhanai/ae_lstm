...
class StaticAttentionEstimatorModel(tf.keras.Model):
    ...
    def compute_loss(self, y, y_pred, input_noise):
        ...
        y_pred_img = y_pred[0]
        loss = tf.reduce_mean(tf.square(y_aug - y_pred_img))
        return loss
...
def model_decoder(output_shape, name='decoder'):
    ...
    # x = tf.keras.layers.Dense(h*w*64, activation='selu')(input_feature)
    # x = tf.keras.layers.BatchNormalization()(x)
    ...
    # decoded_roi_params = tf.keras.layers.Dense(6, activation='selu')(x)
    # decoder = keras.Model(input_feature, [decoded_roi_images, decoded_roi_params], name=name)

    decoder = keras.Model([input_feature], decoded_img, name=name)    
    decoder.summary()
    return decoder

def model_roi_ae(input_image_shape, name='roi_ae'):
...
def embed_roi_images(args):
    roi_img1, roi_img2, r1, r2, bg_img = args
    ...
    def aux_fn(args):
        ...
        resized_roi_img = tf.image.resize(roi_img, (h,w))
        padded_roi_img = tf.image.pad_to_bounding_box(resized_roi_img, y, x, input_image_size[0], input_image_size[1])
        d = 1.0 # dummy
        mask = (resized_roi_img + d) / (resized_roi_img+ d)
        fg_mask = tf.image.pad_to_bounding_box(mask, y, x, input_image_size[0], input_image_size[1])
        return tf.stack([padded_roi_img, fg_mask])

    res1 = tf.transpose(tf.map_fn(fn=aux_fn, elems=(roi_img1, r1), dtype=tf.float32), [1,0,2,3,4])
    padded_roi_img1 = res1[0]
    fg_mask1 = res1[1]
    res2 = tf.transpose(tf.map_fn(fn=aux_fn, elems=(roi_img2, r2), dtype=tf.float32), [1,0,2,3,4])
    padded_roi_img2 = res2[0]
    fg_mask2 = res2[1]

    fg_mask = tf.math.maximum(fg_mask1, fg_mask2)
    bg_mask = 1.0 - fg_mask

    merged_img = fg_mask1 * padded_roi_img1 + fg_mask2 * padded_roi_img2 # + bg_mask * bg_img
    return merged_img
...
def model_static_attention_estimator(input_image_shape, noise_stddev=0.2, use_color_augmentation=False, use_geometrical_augmentation=True, name='static_attention_estimator'):
    ...
    encoded_img1 = model_roi_ae(input_image_shape, name='roi_encoder1')(x)
    encoded_img2 = model_roi_ae(input_image_shape, name='roi_encoder2')(x)
    roi_params = model_roi_estimator(input_image_shape)(image_input)
    ...
    merged_img = tf.keras.layers.Lambda(embed_roi_images)([encoded_img1, encoded_img2, r1, r2, image_input])

    merged_img = model_roi_ae(input_image_shape, name='predictor')(merged_img)
    ...
    m = StaticAttentionEstimatorModel(inputs=[image_input, input_noise],
                                          outputs=[merged_img, roi_params, encoded_img1, encoded_img2], name='static_attention_estimator')
    m.summary()
    return m
...
def prepare_for_test_sae(cp='ae_cp.reaching-real.static_attention_estimator.20220607195210'):
    val_ds = Dataset(dataset, joint_range_data=joint_range_data)
    val_ds.load(groups=val_groups, image_size=input_image_size)
...
# r1 = np.array([[0.2,0.2,0.8,0.8],
#                [0.1,0.2,0.7,0.8],
#                [0.3,0.3,0.5,0.5],
#                [0.0,0.1,0.4,0.6]])
# r2 = np.array([[0.5,0.5,0.8,0.8],
#                [0.4,0.5,0.7,0.8],
#                [0.2,0.4,0.5,0.5],
#                [0.6,0.5,0.9,0.8]])
