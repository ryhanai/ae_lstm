...
def model():
    encoder_input = tf.keras.Input(shape=(height, width, 3))
    ...
    d4 = tf.keras.layers.Dense(latent_dim, activation='selu')(x)

    x = tf.keras.layers.Flatten()(c3)
    d3 = tf.keras.layers.Dense(latent_dim, activation='selu')(x)

    x = tf.keras.layers.Flatten()(c2)
    d2 = tf.keras.layers.Dense(latent_dim, activation='selu')(x)

    encoder_output = tf.keras.layers.concatenate([d4, d3, d2])
    encoder = tf.keras.Model(encoder_input, encoder_output, name='encoder')
    encoder.summary()

    decoder_input = tf.keras.Input(shape=(latent_dim*3))
    dd4 = tf.keras.layers.Lambda(lambda x: x[:,:latent_dim], output_shape=(64))(decoder_input)
    dd3 = tf.keras.layers.Lambda(lambda x: x[:,latent_dim:latent_dim*2], output_shape=(64))(decoder_input)
    dd2 = tf.keras.layers.Lambda(lambda x: x[:,latent_dim*2:latent_dim*3], output_shape=(64))(decoder_input)
...
with strategy.scope():
    ...
    autoencoder_input = tf.keras.Input(shape=(height, width, 3), name='img')
