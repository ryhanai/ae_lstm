...
def model_lstm(time_window_size, image_vec_dim, dof, use_stacked_lstm=True, name='lstm'):
    ...
    x = tf.keras.layers.LSTM(state_dim)(x)
    ...
    m = keras.Model([imgvec_input, joint_input], [predicted_ivec, predicted_jvec], name=name)
    m.summary()
    return m
