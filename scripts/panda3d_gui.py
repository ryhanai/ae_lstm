...
class MyApp(BaseRenderer, ShowBase):
    ...
    def setupScene(self, client, scene_file="kitting_scene.yaml"):
        self.rootdir = "../"
        pb.setAdditionalSearchPath(self.rootdir)
        ...
        # self.robot = pb.loadURDF(robot_desc['robot_model'], base_pose['xyz'], pb.getQuaternionFromEuler(base_pose['rpy']), useFixedBase=True)
        ...
        self.cabinet = pb.loadURDF(d['object'], d['xyz'], pb.getQuaternionFromEuler(d['rpy']), useFixedBase=True, useMaximalCoordinates=True)
        self.objects = {}
        for d in scene_desc['objects']:
            self.objects[d['name']] = pb.loadURDF(d['object'], d['xyz'], pb.getQuaternionFromEuler(d['rpy']), useFixedBase=d['static'], useMaximalCoordinates=True)
