...
class VoxelHead(nn.Module):
    def __init__(
        ...
        self.scratch.output_conv2 = nn.Sequential(
            ...
            nn.Conv2d(head_features_2, 30, kernel_size=1, stride=1, padding=0),  # forcemap z-resolution is 30
            nn.ReLU(True),
            nn.Identity(),
        )
    ...
    def forward(self, out_features, patch_h, patch_w):
        ...
        out = F.interpolate(out, (80, 80), mode="bilinear", align_corners=True)  # forcemap x,y resolutions are 80
        out = self.scratch.output_conv2(out)
        
        return out


class ForceEstimationV5(nn.Module):
    def __init__(
        self, 
        device=0,
        ...
        super(ForceEstimationV5, self).__init__()
        ...
        self.resizer = T.Compose(
            [
                T.Resize([518, 742], antialias=True),
    ...
    def forward(self, x):
        ...
        # x, (h, w) = self.image2tensor(x, input_size)  # This doesn't work on GPU

        # resize
        # x = F.interpolate(x, size=(518, 742), mode='area')
        # normalize
        # parepare for net???
        ...
        force = self.head(features, patch_h, patch_w)
        force = F.relu(force)
        # force = F.sigmoid(force)  # in the previous forcemap prediction, the output was sigmoid
    ...
    def infer_image(self, raw_image, input_size=518):
        ...
        force = self.forward(image)
        
        # Maybe, this is not needed?
        # force = F.interpolate(force[:, None], (h, w), mode="bilinear", align_corners=True)[0, 0]
